...
while( $part1_row = mysqli_fetch_object($part1_result) ) {
	...
	while( $part2_row = mysqli_fetch_object($part2_result) )
	{
		...
		depth2[i][j] = "<?=$part2_row->part_name_k;?>" ;
		...
		while( $part3_row = mysqli_fetch_object($part3_result) )
		{
			...
			depth3[i][j][k] = '<?=$part3_row->part_name_k?>' ;
...
function goodsRanking(part_idx){
	...
	window.open("product_k_ranking.php?part_idx="+part_idx,"","scrollbars=no, width=700, height=600, top="+wintop+", left="+winleft+"");
}
//-->
</script>


	<h4 class="page-header">제품목록(국문)</h4>
	...
	<!-- <div class="row">
	  <a href="javascript:cate_pop('cs_goods_k','icon');" class="btn btn-primary btn-xs active">추가표시</a>
	</div><br> -->
	...
	<col width="10%" title="메인노출1">
	<col width="10%" title="메인노출2">
	...
	<tr>
		...
		<td class="text-center">
		<a href="#none" onclick="window.open('table_k1.php?code=<?=$row['code']?>','','width=1000,height=600,scrollbars=yes');" class="btn btn-sm btn-primary">등록</a>
		</td>
		<td class="text-center">
		<a href="#none" onclick="window.open('table_k2.php?code=<?=$row['code']?>','','width=1000,height=600,scrollbars=yes');" class="btn btn-sm btn-primary">등록</a>
		</td>
		<td class="text-center">
		<a href="#none" onclick="window.open('product_k_file.php?code=<?=$row['code']?>','','width=1000,height=600,scrollbars=yes');" class="btn btn-sm btn-primary">등록</a>
		</td>
		<td class="text-center"><a href="./product_k_edit.php?returnURL=<?=urlencode($_SERVER['REQUEST_URI'])?>&idx=<?=$row['idx']?>" class="btn btn-default btn-sm">수정하기</a></td>
	</tr>
	<?
		$listNo--;
		}
	...
	<table class="table">
		<tr>
			<td class="text-center"><a href="./product_k_add.php" class="btn btn-primary">등록하기</a></td>
		</tr>
	</table>
