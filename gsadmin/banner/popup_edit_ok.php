...
if( $_POST['title_bar'] ) {	
	...
	if( $display == 0 ) {
		...
		@unlink("../../data/designImages/".$row->popup_images2); $popup_images2="";
		...
		if( $_FILES['popup_images2']['size'] > 0 ) {
			...
			$popup_images2=$row->popup_images2;
		}
		$content = "";
		...
		for($i = 1; $i<=5; $i++){
			...
			if($_POST[$del_form]=="y"){
				if($i==1){
					...
					$file_query.="bbs_file$i='',";
				}
				@unlink( $ROOT_DIR."/data/designImages/".$bbs_data_stat->$file_form );
			}else{
	...
	$sql="kind='$kind',
		...
		link_url3='$link_url3',
		link_url4='$link_url4',
		link_url5='$link_url5' where idx='$idx'";
